...
const TabIndicator = ({ status }: { status: "none" | "loading" | "loaded" | "error" }) => {
  ...
  if (status === "loading") return <Loader2 className="h-3 w-3 animate-spin ml-1.5 text-primary" />;
  if (status === "loaded") return <Check className="h-3 w-3 ml-1.5 text-green-500" />;
  if (status === "error") return <AlertCircle className="h-3 w-3 ml-1.5 text-destructive" />;
  return null;
};
...
const Index = ({ initialDomain }: { initialDomain?: string }) => {
  ...
  // Scroll to top when landing on fresh home page (no domain)
  useEffect(() => {
    if (!initialDomain) {
  ...
  // Support initialDomain (pseudo-static route)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-grid-light flex flex-col">
      ...
      >
        <div
          className={`relative container mx-auto px-3 sm:px-4 transition-all duration-300 ${
            hasDomain ? "py-2 sm:py-3" : "py-4 sm:py-8 md:py-20"
          }`}
        >
          {/* Title — only shown when no query active */}
          {!hasDomain && (
      ...
      {hasDomain && (
        <div
          ref={resultsRef}
          className="container mx-auto px-2 sm:px-4 py-4 md:py-8 max-w-full overflow-hidden flex-1"
        >
          <Tabs
            value={activeTab}
          ...
          >
            <TabsList className="grid w-full grid-cols-4 mb-3 sm:mb-4 md:mb-10 bg-card/30 backdrop-blur-md border border-border/50 p-1 sm:p-2 h-auto rounded-xl sm:rounded-2xl shadow-lg">
              <TabsTrigger
                value="whois"
